...
/* ---- NORMALIZE FIELD NAMES FOR APACS MAPPING ----
   DB stores field_name as e.g. "MOVEMENT TYPE", "CARGO",
   "CLIENT NAME", "WEIGHT UNIT" (uppercase, with spaces).
   APACS expects movementType / cargo / clientName / weightUnit.
   Normalize by lowercasing and stripping spaces so the lookup
   is resilient to how the admin named the dynamic fields. */
$apacsNorm = [];
foreach ($apacsFields as $name => $val) {
    $apacsNorm[strtolower(preg_replace('/\s+/', '', $name))] = $val;
}
...
"movementType" => strtolower($apacsNorm['movementtype'] ?? ""),
    "cargo"           => $apacsNorm['cargo'] ?? "",

    "clientName"      => $apacsNorm['clientname'] ?? "",
...
"weightUnit" => strtolower($apacsNorm['weightunit'] ?? "")
);
